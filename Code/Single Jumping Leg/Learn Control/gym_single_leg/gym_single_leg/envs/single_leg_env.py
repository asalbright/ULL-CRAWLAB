...
class SingleLegEnv(gym.Env):
    ...
    def _connect_to_pybullet(self, showGUI):
        if showGUI:
            ...
            if (egl):
                self.eglPluginId = p.loadPlugin(egl.get_filename(), "_eglRendererPlugin")
            else:
                self.eglPluginId = p.loadPlugin("eglRendererPlugin")
            if self.eglPluginId>=0:
                print("\n", "Using GPU hardware (eglRenderer)")  
            else:
                print("\n", "Using CPU renderer (TinyRenderer)")
            # p.configureDebugVisualizer(p.COV_ENABLE_RENDERING, physicsClientId=client)
            p.resetSimulation(physicsClientId=client)
            p.configureDebugVisualizer(p.COV_ENABLE_GUI, physicsClientId=client)
            p.setAdditionalSearchPath(pybullet_data.getDataPath(), physicsClientId=client)
    ...
    def close(self):
        # If we loaded plugin, close it
        if not self.show_GUI:
            p.unloadPlugin(self.eglPluginId)
