...
class SingleJumpingLeg:
    def __init__(self, 
        ...
        # Motor dynamics
        self.max_motor_position = max_motor_pos
        self.max_motor_velocity = max_motor_vel
    ...
    def modify_design(self, des_params):
        pass
...
class SingleJumpingLegFlexible(SingleJumpingLeg):
    def __init__(self, 
                 client, 
                 robot_location,
                 max_motor_pos=np.deg2rad(45),
                 max_motor_vel=np.deg2rad(90), 
                 max_motor_force=100,
                 position_gain=1,
                 velocity_gain=0.01,
        ...
        self.position_gains = np.array([position_gain for ii in range(len(self.flex_joints))])
        self.velocity_gains = np.array([velocity_gain for ii in range(len(self.flex_joints))])
        self.pos_gain_nom = position_gain
        self.vel_gain_nom = velocity_gain
        self.max_flex_position = flex_max_pos
    ...
    def apply_action(self, action):
        ...
        forces = self.max_flex_position * self.position_gains
    ...
    def modify_design(self, params:dict=None, perc=0.5):
        # If the parameters are passed, update the gains
        if not params is None:
            self.position_gains = np.array([params[f'kp_{ii}'] for ii in range(len(self.flex_joints))])
            self.velocity_gains = np.array([params[f'kd_{ii}'] for ii in range(len(self.flex_joints))])

        # Update the gains randomly within a range
        else:
            max_pos_gain = self.pos_gain_nom + perc * self.pos_gain_nom
            min_pos_gain = self.pos_gain_nom - perc * self.pos_gain_nom
            self.position_gains = np.array([np.random.uniform(min_pos_gain, max_pos_gain) for ii in range(len(self.flex_joints))])

            max_vel_gain = self.vel_gain_nom + perc * self.vel_gain_nom
            min_vel_gain = self.vel_gain_nom - perc * self.vel_gain_nom
            self.velocity_gains = np.array([np.random.uniform(min_vel_gain, max_vel_gain) for ii in range(len(self.flex_joints))])
