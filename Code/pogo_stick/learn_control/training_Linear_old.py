...
# "RewardHeightPunishPowerLinear" , "RewardHeightPunishPowerNonlinearCubed" , "RewardHeightPunishPowerNonlinearCubedSqrt"
REWARD_FUNCTION = "RewardHeightPunishPowerLinear"
...
save_path = Path.cwd()
save_path = save_path.joinpath(f'Training_Linear_Data')
...
for weight in OMEGA_X:
    ...
    for trial in range(NUM_TRIALS):
        ...
        # set the callbacks
        evalCallback = EvalCallback(eval_env=eval_env, n_eval_episodes=1, eval_freq=EVALUATION_FREQ, best_model_save_path=models_path, best_model_save_name=f'Agent_{int(weight * 100)}_Linear', deterministic=True, render=False)

        # create the model
        # open tensorboard with the following bash command: tensorboard --logdir ./logs/

        buffer_size = N_TRAINING_STEPS + 1
        model = TD3("MlpPolicy", env, verbose=1, tensorboard_log=logs_path, buffer_size=buffer_size, learning_starts=ROLLOUT, seed=trial_seed, gamma=0.995)

        # train the agent
        
        model.learn(total_timesteps=N_TRAINING_STEPS, callback=evalCallback, tb_log_name=f'Agent{int(weight * 100)}_Linear_log')
end = time.time()

total = end - start

print(f'Time to Train: {total}')