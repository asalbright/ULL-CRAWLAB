...
REWARD_FUNCTION = "RewardHeightPunishPowerNonlinearCubedSqrt"
...
data_name = f'NonlinCubeSqrt{JUMP_TYPE}'
save_path = Path.cwd()
save_path = save_path.joinpath(f'training_{data_name}')
...
def trainAgents(weight):
    ...
    for trial in range(NUM_TRIALS):
        ...
        # set the callbacks
        evalCallback = EvalCallback(eval_env=eval_env, n_eval_episodes=1, eval_freq=EVALUATION_FREQ, best_model_save_path=models_path, best_model_save_name=f'{data_name}_{int(weight * 100)}', deterministic=True, render=False)
        ...
        model.learn(total_timesteps=N_TRAINING_STEPS, callback=evalCallback, tb_log_name=f'{data_name}_{int(weight * 100)}_log')
...
if __name__ == "__main__":
    start = time.time()
    runMultiprocessTraining(trainAgents, OMEGA_X, 3)
    end = time.time()
    total = end - start
