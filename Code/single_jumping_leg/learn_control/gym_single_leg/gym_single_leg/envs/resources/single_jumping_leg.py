...
class SingleJumpingLeg:
    def __init__(self, 
        ...
        num = p.getNumJoints(self.leg, 
                             physicsClientId=self.client)
        joint_info = []
        for ii in range(num): 
            # Append the the joint info list the info of the ii'th joint
            joint_info.append(p.getJointInfo(self.leg, 
                                             ii,
                                             physicsClientId=self.client))
            ...
            if "J_C" in joint_name:
                # Add the continuous joints index to the motor joints list
                ...
                p.changeVisualShape(self.leg,
                                    ...
                                    rgbaColor=RED,
                                    physicsClientId=self.client)
            # If the ii'th joints name has a "J_P" in it, it is prismatic joint
            elif "J_P" in joint_name:
                # Add the prismatic joint to the slider joints list
                ...
                p.changeVisualShape(self.leg,
                                    ...
                                    rgbaColor=GREEN,
                                    physicsClientId=self.client)
            # If the ii'th joints name has a "J_F" in it, it is a flex joint
            elif "J_F" in joint_name:
        ...
        # Set the prismatic joint(s) to be free sliding with some friction
        if len(self.sliding_joints) > 0:
            p.setJointMotorControl2(self.leg, self.sliding_joints[0],
                                p.VELOCITY_CONTROL,
                                force=0.0,
                                physicsClientId=self.client)
    ...
    def apply_action(self, action):
        ...
        if self.control_mode == "POSITION_CONTROL":
            ...
            # Apply the position control to the motors
            num_joints = p.getNumJoints(self.leg, 
                                        physicsClientId=self.client)
    ...
    def get_height(self, link=None):
        ...
        if link is None: link = self.sliding_joints[0]
